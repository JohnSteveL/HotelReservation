...
class DataPreprocessor:
    ...
    def select_features(self,df):
        try:
            logger.info("Starting Feauture Selection")
            X = df.drop(columns="booking_status")
            y = df['booking_status']
            ...
            features_selected = feature_importance_df.sort_values(by="importance" , ascending=False)
            num_features_to_select = self.config["data_processing"]["no_of_features"]
            top_features_selected = features_selected["feature"].head(num_features_to_select).values

            logger.info(f"Features selected : {top_features_selected}")
            selected_df = df[top_features_selected.tolist() + ["booking_status"]]
